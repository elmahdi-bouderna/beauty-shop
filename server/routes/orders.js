...
function createWordExport(orders, res) {
  try {
    ...
    if (fs.existsSync(logoPath)) {
      ...
      logoHtml = `<img src="data:image/png;base64,${logoBase64}" style="height: 100px; display: block; margin: 0 auto;">`;
    }
    ...
    let html = `
      ...
      <head>
        ...
        <style>
          ...
          h1, h2 { text-align: center; color: #3b82f6; }
          ...
          .order-header { background-color: #f3f4f6; padding: 10px; margin-top: 30px; border-left: 4px solid #3b82f6; }
          .order-details { margin-bottom: 10px; }
          .total-row { font-weight: bold; background-color: #f3f4f6; color: #3b82f6; }
          .page-break { page-break-before: always; }
          .status { padding: 3px 8px; border-radius: 3px; color: white; display: inline-block; font-size: 12px; }
      ...
      <body>
        ...
        <p><strong>Nombre total de commandes:</strong> ${orders.length}</p>
    `;
    ...
    // Add rows for each order in summary
    orders.forEach(order => {
      try {
        ...
        console.error(`Error processing order ${order.id} in Word summary:`, error);
      }
    });
    ...
    // Add detailed section for each order
    orders.forEach((order, index) => {
      try {
        ...
        html += `
          <div class="order-header">
            <h2>Commande #${order.id} <span class="status ${statusClass}">${statusText}</span></h2>
          </div>
    ...
    html += `
      <div class="footer">
        Beauty Shop - Tous droits réservés
      </div>
    `;
