    ...
    if (req.body.colors) {
      ...
      for (let i = 0; i < colors.length; i++) {
        ...
        if (colorIndex !== undefined && req.files[`colorImage_${colorIndex}`]) {
          // Delete old image if it exists and is different from product image
          if (color.id && !color.isNew && color.image && color.image !== image) {
            try {
              const oldImagePath = path.join(__dirname, '..', color.image);
              if (fs.existsSync(oldImagePath)) {
                fs.unlinkSync(oldImagePath);
              }
            } catch (err) {
              console.error(`Error deleting old color image for color ${color.id}:`, err);
            }
          }
          
          // Set new image path
          colorImage = `/uploads/${req.files[`colorImage_${colorIndex}`][0].filename}`;
        }
